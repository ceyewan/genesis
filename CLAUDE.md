...
| **Level 2: Business** | `cache`, `idgen`, `idem`, `dlock`, `mq` | 业务能力封装 |
| **Level 1: Infrastructure** | `connector`, `db` | 连接管理，底层 I/O |
| **Level 0: Base** | `clog`, `config`, `metrics`, `trace`, `xerrors` | 框架基石，被所有上层依赖 |
...
- 格式化使用 `gofumpt`，通过 `golangci-lint` 强制执行
- Import 使用 `goimports` 格式，分三组：标准库 / 第三方 / 内部包（`github.com/ceyewan/genesis/...`）
- 使用 Go 1.26+ 工具链时，定期运行 `go fix ./...`；提交前至少运行 `make modernize-check`
...
- 错误使用 `xerrors` 包装，禁止 `errors.New` / `fmt.Errorf`
- 错误要显式返回，不要忽略
...
4. **改后必须验证**：改动后运行对应测试或 lint；如修改 Go 代码，同时运行 `make modernize-check`。
