...
`internal/testkit` 是 Genesis 仓库内部的测试辅助包，目标不是提供一套庞大的测试框架，而是把项目里反复出现的测试基础设施和通用 helper 沉淀成统一入口。它不属于公共 module API，外部项目不能导入。

接口文档见 `go doc -all ./internal/testkit`。这个包没有单独的设计 blog，因为它是测试辅助包，不是面向生产接入的组件。
