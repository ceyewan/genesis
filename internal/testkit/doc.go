// Package testkit 提供 Genesis 仓库内部组件测试所需的通用 helper。
//
...
//	defer cancel()
//
// 所有 helper 都以 *testing.T 为生命周期锚点，通过 t.Cleanup 自动回收
// 容器、连接器和临时资源。需要文件持久化的 SQLite helper 会使用 t.TempDir()
// 创建临时目录。
//
// 这个 internal 包不是 Genesis 公共模块契约；外部项目应在自己的测试模块中维护
// 所需的 Testcontainers fixture。它不会替测试自动建表、造业务数据或模拟完整调用链，
// 这些控制权仍应由各组件测试自行保留。
package testkit
