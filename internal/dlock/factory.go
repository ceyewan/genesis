...
func NewEtcd(conn connector.EtcdConnector, cfg *types.Config, logger clog.Logger) (types.Locker, error) {
	return etcd.New(conn, cfg, logger)
}
...
func New(conn interface{}, cfg *types.Config, logger clog.Logger) (types.Locker, error) {
	switch cfg.Backend {
	case types.BackendRedis:
		if c, ok := conn.(connector.RedisConnector); ok {
			return NewRedis(c, cfg, logger)
		}
