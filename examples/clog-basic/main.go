...
func basicConsoleExample() {
	fmt.Println("--- Example 3: Console Format ---")
...
func basicContextExample() {
	...
	logger := clog.Must(&clog.Config{
		...
		ContextFields: []clog.ContextField{
			{
				Key:       "request_id", // Context key
				FieldName: "request_id", // 输出字段名
				Required:  false,
			},
			{
				Key:       "user_id",
...
func basicNamespaceExample() {
	fmt.Println("--- Example 8: Namespace Usage ---")
...
func componentVsNamespaceExample() {
	fmt.Println("--- Example 9: Component vs Namespace ---")
