...
- **核心职责**：在数据库连接器的基础上提供 GORM ORM 功能和分库分表能力
- **支持数据库**：MySQL、SQLite
- **设计原则**：
...
// 2. 创建 DB 组件
database, _ := db.New(sqliteConn, &db.Config{}, db.WithLogger(logger))
...
```go
type Config struct {
    EnableSharding bool   // 是否开启分片特性
    ShardingRules  []ShardingRule  // 分片规则配置列表
}
```
...
database, err := db.New(conn, cfg, db.WithLogger(logger))

// WithMeter 注入指标收集器
database, err := db.New(conn, cfg, db.WithMeter(meter))
...
3. **注入依赖**：通过 `WithLogger` 和 `WithMeter` 注入可观测性组件
