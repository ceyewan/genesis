...
Genesis `breaker` 是治理层组件，定位很明确：它服务于 gRPC 客户端调用链，在下游出现系统性异常时快速失败，避免请求继续堆积和故障扩散。这篇文章不讲“怎么把 gobreaker 包一层”，而是重点解释三个设计判断：为什么要按 key 独立熔断，为什么必须区分系统错误和业务错误，以及 fallback 如何在拒绝时生成替代结果。
...
- `WithFallback` 只在 breaker 拒绝请求时运行，并可返回替代业务结果或替代错误。
...
配置有六个字段：`MaxKeys` 限制单实例 key 基数，`MaxRequests` 控制半开阶段允许通过的探测请求数，`Interval` 控制闭合状态下计数的重置周期，`Timeout` 决定打开状态持续多久，`FailureRatio` 和 `MinimumRequests` 一起决定何时触发熔断。新版本里，Genesis 对配置加了基础校验：`FailureRatio` 必须位于 `(0, 1]`，`Interval` 不能为负，`Timeout` 也不能为负。零值仍然保留为“使用默认值”的语义。
...
组件内部最重要的概念不是“一个 breaker”，而是“每个 key 一个 breaker”。实现用 mutex 保护普通 map，每个 key 各自拥有统计、状态和迁移过程，不会互相污染，并通过 `MaxKeys` 拒绝无界基数。这个设计让同一个组件实例既可以做服务级熔断，也可以做方法级熔断，差别仅仅在于 key 如何生成。
...
另一个重要取舍是 `fallback`。`FallbackFunc` 返回 `(any, error)`，只在 breaker 拒绝执行时调用，因此可以返回缓存值、默认响应或改写后的错误。通用 `Execute` 会把替代结果直接返回；gRPC 拦截器只有在结果与 reply 类型兼容时才能应用它，业务仍应为每个方法提供类型正确的降级值。
...
如果你的目标是“当 breaker 打开时直接返回缓存对象”，可以让 fallback 返回与当前 gRPC reply 类型兼容的 protobuf 响应。拦截器会复制该结果；类型不兼容时返回 `codes.Internal`，而不是返回成功却留下空 reply。更复杂的缓存选择、重试和业务降级编排仍应放在上层。
