...
- 借用模型明确所有权：连接器自己关闭底层连接，上层组件（cache、dlock、mq）只借用客户端，Close 是 no-op
...
### 6.2 为什么上层组件的 Close 是 no-op

`cache`、`dlock` 等组件的 `Close()` 方法通常什么都不做，或者只关闭它们自己拥有的资源（如 otter 缓存的后台 goroutine），而不关闭注入进来的连接器。
